...
const getRunes = function() {
    ...
    for (let item of checkAlch) {
        ...
        newRow.children[2].innerHTML = rsapidata[item.id].price + item.alchval - rsapidata[item.id].price - alchemiserprice;
        newRow.children[2].dataset.value = (rsapidata[item.id].price + item.alchval - rsapidata[item.id].price - alchemiserprice).toLocaleString();
